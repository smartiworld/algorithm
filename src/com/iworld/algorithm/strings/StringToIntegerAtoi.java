...
public class StringToIntegerAtoi {
    ...
    public int myAtoi(String s) {
        ...
        boolean isHaveDigit = false;
        for (int i = 0; i < chars.length; i++) {
            if (chars[i] == ' ') {
                // 遇见空格 如果前面已经有了正负号或者数字 需要结束 没有则继续
                if (isHaveDigit || isHaveFlag) {
                    break;
                }
                continue;
            }
            // 遇见正负号 如果前面已经遇见正负号或者数据 结束处理 否则设置符号位 继续
            if (chars[i] == '-') {
                if (isHaveDigit || isHaveFlag) {
                    break;
                }
            ...
            isHaveDigit = true;
            long tmp = ans * 10 + (chars[i] - '0');
            if (positive && tmp > Integer.MAX_VALUE) {
                return Integer.MAX_VALUE;
            }
            if (!positive && tmp > Integer.MAX_VALUE + 1L) {
