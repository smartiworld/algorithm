...
public class LRUCacheCusMap {
    ...
    public LRUCacheCusMap(int capacity) {
        ...
        n |= n >>> 8;
        n |= n >>> 16;
        tableSize = (n < 0) ? 1 : (n >= Integer.MAX_VALUE) ? Integer.MAX_VALUE : n + 1;
        tables = new Entry[tableSize];
    }
    
    public int get(int key) {
        Entry entry = tables[hash(key) & (tableSize - 1)];
        while (entry != null) {
            if (entry.key == key) {
                break;
            }
    ...
    public void put(int key, int value) {
        Entry entry;
        int index;
        if ((entry = tables[index = ((tableSize - 1) & hash(key))]) == null) {
            tables[index] = createEntry(key, value);
        } else {
            while (entry != null) {
                if (entry.key == key) {
                    entry.val = value;
    ...
    private void removeFirst() {
        if (head != null) {
            ...
            Entry removeEntry = head;
            int index = hash(removeEntry.key) & (tableSize - 1);
            // 删除hashmap
            Entry entry = tables[index];
            if (entry != null) {
                if (entry.key == removeEntry.key) {
                    tables[index] = entry.next;
                } else {
                    while (entry.next != null) {
                        if (entry.next.key == removeEntry.key) {
    ...
    static class Entry {
        ...
        Entry(int key, int value, Entry before, Entry after, Entry next) {
            this.key = key;
            this.val = value;
            this.before = before;
            this.after = after;
    ...
    public static void main(String[] args) {
        ...
        int i = 0;
        try {
            Class<LRUCacheCusMap> lruCacheClass = LRUCacheCusMap.class;
            Constructor<LRUCacheCusMap> declaredConstructor = lruCacheClass.getDeclaredConstructor(int.class);
            LRUCacheCusMap lruCacheUseMap = declaredConstructor.newInstance(2);
